...
p_lead      <- make_three_plots(nhanes_filtered, "LBXBPB", "Blood Lead")
p_cadmium   <- make_three_plots(nhanes_filtered, "LBXBCD", "Blood Cadmium")
p_manganese <- make_three_plots(nhanes_filtered, "LBXBMN", "Blood Manganese")
# Plot the 3 numeric variables (original and cleaned versions) on a 3x3 grid
...
# Apply helper function to blood pressure data(integers but treat as numeric variables for modeling) but don't log transform
...
# Function to summarize one variable by sex
summarize_ttest <- function(var_name, data, group_var) {
  
  # Run t-test
  t_res <- data %>%
    t_test(as.formula(paste(var_name, "~", group_var)))
  
  # Calculate effect size (Cohen's d)
  d_res <- data %>%
    cohens_d(as.formula(paste(var_name, "~", group_var)))
  ...
  summary_tbl <- group_stats %>%
    mutate(
      ...
      p_value = round(t_res$p, 3),
      cohen_d = round(d_res$effsize, 3)
    )
  
  # Add descriptor column for pivoting
...
numeric_vars <- c("LBXBPB_clean_log", "LBXBCD_clean_log", "LBXBMN_clean_log", "BPXSY1_clean", "BPXDI1_clean")
group_var <- "RIAGENDR"

# Apply function to all numeric variables
ttest_results_long <- lapply(numeric_vars, summarize_ttest,
                             data = nhanes_clean, group_var = group_var) %>%
  reduce(left_join, by = "descriptor")

# Display result for all vars as tibble
ttest_results_long
...
**Dataset Description:** The null hypothesis is that the proportion of adults with high BP is the same across all racial/ethnic groups.
