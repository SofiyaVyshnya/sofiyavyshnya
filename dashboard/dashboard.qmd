...
# Heavy Metals {orientation="columns"}
## Column {width="60%"}
### Row {height="33.33%"}
#### Column {width="40%"}
```{r}
#| title: Blood Lead Concentration

ggplot(nhanes_data, aes(x = "", y = LBXBPB)) +
  geom_violin(fill = "#4b85a8", color = "black", alpha = 0.7)  +
  coord_flip() +
  ...
  theme_minimal() +
  theme(
    axis.text = element_text(size = 35),       # tick labels
...
#### Column {width="60%"}
```{r}
#| title: Blood Lead Concentration by Sex 
ggplot(nhanes_data, aes(x = as.factor(RIAGENDR), y = LBXBPB, fill = as.factor(RIAGENDR))) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) +
  scale_fill_manual(values = c("#aba597", "#e1b200")) +
  coord_flip() +
  labs(
    x = "Sex",
    y = "\nConcentration (μg/dL)",
...
#### Column {width="40%"}
```{r}
#| title: Blood Cadmium Concentration

ggplot(nhanes_data, aes(x = "", y = LBXBCD)) +
  geom_violin(fill = "#e1b200", color = "black", alpha = 0.7)  +
  coord_flip() +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) + # optional: overlay boxplot
  labs(
    x = "",
...
#### Column {width="60%"}
```{r}
#| title: Blood Lead Concentration by Age

ggplot(nhanes_data, aes(x = as.factor(age_group), y = LBXBPB, fill = as.factor(age_group))) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) +
  scale_fill_manual(values = c("#182139", "#4b85a8", "#aba597", "#e1b200", "#d98100")) +
  labs(
    x = "",
    y = "Concentration\n(μg/dL)",
    title = ""
...
#### Column {width="40%"}
```{r}
#| title: Blood Manganese Concentration

ggplot(nhanes_data, aes(x = "", y = LBXBMN)) +
  geom_violin(fill = "#aba597", color = "black", alpha = 0.7) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA)  +
  coord_flip() +
  labs(
    x = "",
...
#### Column {width="60%"}
```{r}
#| title: Blood Lead Concentration by Race/Ethnicity

ggplot(nhanes_data, aes(x = as.factor(RIDRETH1), y = LBXBPB, fill = as.factor(RIDRETH1))) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) +
  scale_fill_manual(values = c("#182139", "#4b85a8", "#aba597", "#e1b200", "#d98100")) +
  coord_flip() +
...
# BP & Heavy Metals {orientation="rows"}
## Row {height="60%"}
```{r}
#| title: Regression Output - Predicting Systolic BP from Demographics and Heavy Metal Exposure

# Model
# Linear regression predicting systolic BP from heavy metal concentration in blood
model <- lm(BPXSY1_clean ~ LBXBPB_clean + LBXBCD_clean + LBXBMN_clean + RIDAGEYR + RIAGENDR + RIDRETH1, data = nhanes_data)
...
# Summarize results: Tidy regression coefficients
model_coefs <- tidy(model) %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  mutate(across(where(is.numeric), round, 3))


# Rename coefficient table columns
...
# Dictionary of predictor descriptions
predictor_info <- tibble::tibble(
  term = c(
    "LBXBPB_clean", "LBXBCD_clean", "LBXBMN_clean",
    "RIDAGEYR", "RIAGENDRFemale", "RIDRETH1Other Hispanic", "RIDRETH1Non-Hispanic White", "RIDRETH1Non-Hispanic Black", "RIDRETH1Other/Multiracial"
  ),
  description = c(
    "Blood Lead (μg/dL)",
    "Blood Cadmium (μg/dL)",
...
# Add descriptions to coefficients and reorder columns
model_coefs_annotated <- model_coefs %>%
  left_join(predictor_info, by = "term") %>%
  mutate(description = ifelse(term == "(Intercept)", "Model Intercept", description)) %>%
  select(term, description, everything())  # Put description right after term

# Rename model statistics
model_stats <- model_stats %>%
  rename(
    `R-squared` = r.squared,
    `Adjusted R-squared` = adj.r.squared,
    `Akaike Info Criterion (AIC)` = AIC,
    `Bayesian Info Criterion (BIC)` = BIC
  )

# Regression Output
datatable(
  model_coefs_annotated,
...
datatable(
  model_stats,
  options = list(dom = 't', paging = FALSE), # simple small table
  rownames = FALSE
)
```
...
## Row {height="40%"}
```{r}
#| title: Regression Output - Relative Importance of Predictors

ggplot(
  model_coefs %>% filter(term != "(Intercept)"),
  aes(x = reorder(term, estimate), y = estimate, fill = estimate > 0)
) +
  geom_col() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  coord_flip() +
  scale_fill_manual(values = c("TRUE" = "#4b85a8", "FALSE" = "#d98100")) +
  labs(
    x = "Predictor",
    y = "Coefficient Estimate",
    title = "Regression Coefficients: Systolic BP Model"
  ) +
  theme_minimal() +
  theme(
...
nhanes_table <- nhanes_data %>%
  ...
  select(
    SEQN, RIDAGEYR, RIAGENDR, RIDRETH1, LBXBPB, LBXBCD, LBXBMN
  ) %>%
  rename(
    `Patient ID` = SEQN,
    `Age (Years)` = RIDAGEYR,
    `Sex` = RIAGENDR,
    `Race/Ethnicity` = RIDRETH1,
    `Blood Lead (μg/dL)` = LBXBPB,
    `Blood Cadmium (μg/dL)` = LBXBCD,
    `Blood Manganese (μg/dL)` = LBXBMN
  )
